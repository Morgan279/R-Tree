...
public interface RTreeNodeSeparator {
    ...
    default <T> RTreeNodeSplitResult<T> split(List<RTreeEntry<T>> entries) {
        ...
        PickSeedsResult<T> pickSeedsResult = pickSeeds(entries);
        ...
        while (!entries.isEmpty()) {
            RTreeEntry<T> nextEntry = RTreeNodeSplitUtil.pickNext(entries, splitList1, splitList2);
            RTreeNodeSplitUtil.pickGroup(nextEntry, splitList1, splitList2).add(nextEntry);
